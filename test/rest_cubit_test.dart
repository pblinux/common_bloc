...
void main() {
  group('Rest cubit', () {
    ...
    blocTest<RestCubit, RestState>('get from api',
    ...
    blocTest<RestCubit, RestState>('post to api',
        act: (cubit) => cubit.post('/posts', body: <String, dynamic>{
    ...
    blocTest<RestCubit, RestState>('update (with put) to api',
        act: (cubit) => cubit.put('/posts/1', body: <String, dynamic>{
              'id': 1,
              'title': 'CommonBlocTest',
              'body': 'This is a new entry',
              'userId': 1
        ...
        expect: () => [isA<LoadingRestState>(), isA<LoadedRestState>()],
        skip: 0);
    blocTest<RestCubit, RestState>('update (with patch) to api',
        act: (cubit) => cubit.patch('/posts/1',
            body: <String, dynamic>{'title': 'CommonBlocTest'}),
        build: () => RestCubit('https://jsonplaceholder.typicode.com'),
        expect: () => [isA<LoadingRestState>(), isA<LoadedRestState>()],
        skip: 0);

    blocTest<RestCubit, RestState>('delete from api',
        act: (cubit) => cubit.delete('/posts/1'),
        build: () => RestCubit('https://jsonplaceholder.typicode.com'),
        expect: () => [isA<LoadingRestState>(), isA<LoadedRestState>()],
        skip: 0);

    blocTest<RestCubit, RestState>('post to api with form data',
        act: (cubit) => cubit.formData('/posts',
  ...
  group('Rest cubit errors', () {
    blocTest<RestCubit, RestState>('bad request response from api',
    ...
    blocTest<RestCubit, RestState>('unauthorized request response from api',
        act: (cubit) => cubit.post('/5e926d0e3100005d00462ca2'),
        build: () => RestCubit('http://www.mocky.io/v2'),
        expect: () => [isA<LoadingRestState>(), isA<ErrorRestState>()],
        skip: 0);

    blocTest<RestCubit, RestState>('forbidden request response from api',
    ...
    blocTest<RestCubit, RestState>('server error request response from api',
        act: (cubit) => cubit.delete('/5e926d2d3100005d00462ca5'),
        build: () => RestCubit('http://www.mocky.io/v2'),
        expect: () => [isA<LoadingRestState>(), isA<ErrorRestState>()],
        skip: 0);

    blocTest<RestCubit, RestState>('unknown request response from api',
        act: (cubit) => cubit.formData('/5eb7c68c3100006a00c8a272',
            body: FormData.fromMap(<String, dynamic>{})),
        build: () => RestCubit('http://www.mocky.io/v2'),
        expect: () => [isA<LoadingRestState>(), isA<ErrorRestState>()],
        skip: 0);
